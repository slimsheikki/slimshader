...
export default function App() {
  ...
  <div className="card-grid">{shaders.map(shader=><button className="shader-card" key={shader.id} onClick={()=>setOpen(shader.id)} aria-label={`Open ${shader.name} editor`}><div className="card-art"><canvas ref={shader.id==='ascii'?thumbnail:toonThumbnail} width={900} height={600}/><span className="card-number">{shader.id==='ascii'?'001 / ASCII':'002 / TOON'}</span><span className="card-tag"><Sparkles size={13}/> LIVE EFFECT</span><span className="card-open"><ArrowUpRight size={24}/></span></div><div className="card-info"><div><span className="eyebrow">{shader.category}</span><h2>{shader.name}</h2><p>{shader.description}</p></div><ArrowRight size={20}/></div></button>)}<div className="next-card"><div className="next-symbol"><Plus size={28} strokeWidth={1}/></div><span className="eyebrow">STILL EXPERIMENTING</span><h2>More to come.</h2><p>The library is just getting started.</p><span className="coming-pill">IN THE MAKING</span></div></div>
