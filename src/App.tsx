...
export default function App() {
  ...
  <main className="gallery"><div className="intro"><div><p className="eyebrow">THE SHADER LIBRARY</p><h1>New ways to see.</h1><p className="subtitle">Turn familiar images into something unexpected.</p></div><div className="collection-count"><span>03</span> EFFECTS & COUNTING</div></div>
  <div className="gallery-toolbar"><span className="selected-tab">All effects <span>3</span></span><span className="local-label"><span/> Made in your browser. Yours to keep.</span></div>
  <div className="card-grid">{shaders.map(shader=><button className="shader-card" key={shader.id} onClick={()=>setOpen(shader.id)} aria-label={`Open ${shader.name} editor`}><div className="card-art"><canvas ref={shader.id==='ascii'?thumbnail:shader.id==='toon'?toonThumbnail:sketchThumbnail} width={900} height={600}/><span className="card-number">{shader.id==='ascii'?'001 / ASCII':shader.id==='toon'?'002 / PAINTERLY TOON':'003 / STORYBOARD'}</span><span className="card-tag"><Sparkles size={13}/> LIVE EFFECT</span><span className="card-open"><ArrowUpRight size={24}/></span></div><div className="card-info"><div><span className="eyebrow">{shader.category}</span><h2>{shader.name}</h2><p>{shader.description}</p></div><ArrowRight size={20}/></div></button>)}<div className="next-card"><div className="next-symbol"><Plus size={28} strokeWidth={1}/></div><span className="eyebrow">STILL EXPERIMENTING</span><h2>More to come.</h2><p>The library is just getting started.</p><span className="coming-pill">IN THE MAKING</span></div></div>
