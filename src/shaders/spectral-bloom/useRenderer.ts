...
import { renderSpectral } from './render';

export function useRenderer(image: ImageBitmap | null, params: SpectralParams, canvas: RefObject<HTMLCanvasElement | null>) {
  ...
  function preview() {
    ...
    if (worker.current) { busy.current = true; worker.current.postMessage({ id, kind: 'preview', params: settings, ...size }); }
      ...
      try { canvas.current.width = size.width; canvas.current.height = size.height; renderSpectral(canvas.current, source, settings); }
