...
export function renderGeometry(target:Surface,image:ImageBitmap,p:GeometryParams,time=0){
 ...
 // A shared smooth displacement field moves image patches and their attached graphics.
 ...
 const nodes=Array.from({length:p.boxes+p.connections?Math.max(8,p.connections+1):0},(_,i)=>{const a=pick(i+30);return warp(a.x,a.y);});
 ...
 for(let i=0;i<p.boxes;i++){
  const a=nodes[i*2%nodes.length],b=nodes[(i*2+5)%nodes.length];
  const x=Math.min(a.x,b.x),y=Math.min(a.y,b.y),bw=Math.max(.08,Math.abs(a.x-b.x)),bh=Math.max(.12,Math.abs(a.y-b.y));
