...
export default function Editor({ onClose }: { onClose: () => void }) {
  ...
  const {rendering,error:renderError,exportPng,exportVideo,playing,setPlaying,progress} = useRenderer(image, params, canvas);
  ...
  async function download(video=false) {
    setError('');setExporting(true);setDownloaded(false);
    try {
      const blob=await (video?exportVideo():exportPng(exportScale)); if(!mounted.current)return;
      const url=URL.createObjectURL(blob), link=document.createElement('a');
      link.href=url;link.download=`${name.replace(/\.[^.]+$/,'').replace(/[^a-z0-9_-]/gi,'-')||'image'}-living-geometry-${video?'animation':exportScale+'x'}.${video?(blob.type.includes('mp4')?'mp4':'webm'):'png'}`;
