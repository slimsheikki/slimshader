...
export default function Editor({ onClose }: { onClose: () => void }) {
  ...
  async function load(file: File | Blob, fileName='Untitled') {
    ...
    try {
      ...
      if(fileName!=='David — Liam Ward')setParams(p=>({...p,shape:'image'}));
  ...
  <div className="editor-body"><div className="workspace-column"><div className="workspace-toolbar"><span className="file-name">{image?name:'YOUR CANVAS'}</span><div>{image&&<>{params.shape==='image'&&<button className={compare?'subtle-button active':'subtle-button'} onClick={()=>setCompare(!compare)} aria-pressed={compare}>Original</button>}<button className="subtle-button" onClick={()=>input.current?.click()} disabled={loading||exporting}><Replace size={14}/> Use image</button></>}</div></div>
