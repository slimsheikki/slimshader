...
import type { AsciiParams } from './model';
import { dimensions } from './model';
import { renderAscii } from './render';

export function useRenderer(image: ImageBitmap | null, params: AsciiParams, canvas: RefObject<HTMLCanvasElement | null>) {
  ...
  function preview() {
    ...
    if (worker.current) { busy.current = true; worker.current.postMessage({ id, kind: 'preview', params: settings, ...size }); }
      ...
      try { canvas.current.width = size.width; canvas.current.height = size.height; renderAscii(canvas.current, source, settings); }
