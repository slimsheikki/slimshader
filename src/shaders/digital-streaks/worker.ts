...
self.onmessage = async ({ data }: MessageEvent<{ id: number; image?: ImageBitmap; params: StreakParams; width: number; height: number; kind: 'preview' | 'export' }>) => {
  try {
    if (data.image) { image?.close(); image = data.image; }
    if (!image) throw new Error('Add an image first.');
    const canvas = new OffscreenCanvas(data.width, data.height);
    renderStreaks(canvas, image, data.params);
    if (data.kind === 'export') {
