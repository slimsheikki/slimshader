...
export function useRenderer(image: ImageBitmap | null, params: HeatedParams, canvas: RefObject<HTMLCanvasElement | null>) {
  ...
  function preview() {
    ...
    if (worker.current) { busy.current = true; worker.current.postMessage({ id, kind: 'preview', params: settings, ...size }); }
      ...
      try { canvas.current.width = size.width; canvas.current.height = size.height; renderHeated(canvas.current, source, settings); }
