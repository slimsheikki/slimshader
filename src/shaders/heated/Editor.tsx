...
export default function Editor({ onClose }: { onClose: () => void }) {
  ...
  async function load(file: File | Blob, fileName='Untitled') {
    ...
    try {
      ...
      if(fileName!=='Shape canvas')setParams(p=>({...p,shape:'image'}));
  ...
  async function download() {
    ...
    try {
      ...
      link.href=url;link.download=`${name.replace(/\.[^.]+$/,'').replace(/[^a-z0-9_-]/gi,'-')||'image'}-heated-${exportScale}x.png`;
  ...
  return <dialog ref={dialog} className="editor-dialog" onCancel={event=>{event.preventDefault();onClose();}} onClick={e=>{if(e.target===dialog.current)onClose();}}><div className="editor-shell"><header className="editor-header"><button className="icon-button back-button" onClick={onClose} aria-label="Back to library"><ArrowLeft size={18}/></button><div className="editor-title"><h2>Heated Shapes</h2><span>Thermal forms</span></div><span className="editor-local"><span/> Local processing</span><button className="icon-button close-button" onClick={onClose} aria-label="Close editor"><X size={19}/></button></header>
  <div className="editor-body"><div className="workspace-column"><div className="workspace-toolbar"><span className="file-name">{params.shape==='image'?name:'PROCEDURAL SHAPE'}</span><div>{image&&<>{params.shape==='image'&&<button className={compare?'subtle-button active':'subtle-button'} onClick={()=>setCompare(!compare)} aria-pressed={compare}>Original</button>}<button className="subtle-button" onClick={()=>input.current?.click()} disabled={loading||exporting}><Replace size={14}/> Use image</button></>}</div></div>
