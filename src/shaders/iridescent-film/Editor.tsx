...
export default function Editor({ onClose }: { onClose: () => void }) {
  ...
  async function download() {
    ...
    try {
      ...
      link.href=url;link.download=`${name.replace(/\.[^.]+$/,'').replace(/[^a-z0-9_-]/gi,'-')||'image'}-iridescent-film-${exportScale}x.png`;
