...
export function useRenderer(image: ImageBitmap | null, params: BlueprintParams, canvas: RefObject<HTMLCanvasElement | null>) {
  ...
  function preview() {
    ...
    if (worker.current) { busy.current = true; worker.current.postMessage({ id, kind: 'preview', params: settings, ...size }); }
      ...
      try { canvas.current.width = size.width; canvas.current.height = size.height; renderBlueprint(canvas.current, source, settings); }
  ...
  async function exportPng(scale: number) {
    ...
    renderBlueprint(c, image, params);
