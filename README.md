...
Glass Panels now back-projects through a broad-centred curved cylindrical profile, with a source-space air gap controlled by Subject distance. Neighbouring panels can sample overlapping image areas, repeating and reversing fragments instead of only enlarging isolated strips. Curved shoulders use wider sample footprints to retain compressed thin features; light, frost, dispersion, placement and gap controls remain available. Reeded Glass starts with 40 narrow ribs for dense repetition. This remains a 2D optical approximation with no recovered depth or hidden surfaces. The previous renderer/settings are retained unregistered in `src/shaders/glass-previous`; the older `glass-legacy` backup also remains hidden.
